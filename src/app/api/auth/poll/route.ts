...
export async function GET(req: NextRequest) {
  try {
    ...
    if (check === "true") {
      const existingSession = await resolveSessionFromCookieStore().catch(() => null);
      if (existingSession) {
        return NextResponse.json(
          {
            status: "ok",
            userId: existingSession.userId.toString(),
            role: existingSession.isAdmin ? "ADMIN" : "USER",
          },
    ...
    // ABSOLUTE GUARD: Never return a non-JSON response from this endpoint.
    return NextResponse.json(
      {
        status: "error",
        error: {
